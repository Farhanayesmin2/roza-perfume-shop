...
const NewProducts = () => {
  ...
  const visiblePerfumes = perfumes.slice(
    currentIndex,
          ...
          <SwiperSlide>
            <div className=" swiper-wrapper  grid grid-cols-1 md:grid-cols-3 gap-6">
              {visiblePerfumes.map((perfume) => (
                <div
                  className="relative swiper-slide hover:scale-105 duration-300"
                  key={perfume.id}
                >
                  <img
                  ...
                  <div className="relative border border-gray-100 bg-white p-6">
                    ...
                    <div className="flex items-center mt-1.5 text-lg text-yellow-400">
                      {" "}
                      <span className=" text-gray-700 mr-2">
                        Rating: {perfume.rating}
                      </span>
                      {Array.from(
                        { length: Math.floor(perfume.rating) },
                        (_, index) => (
                    ...
                    <div className="mt-4 text-center flex">
                      <button className="mt-4 w-full  justify-center items-center text-black bg-gradient-to-r from-[#821D30] via-rose-50 to-[#821D30] hover:bg-[#0077b6] shadow-2xl shadow-white text-lg   font-serif font-semibold py-2 px-4 rounded-full ">
                        Add to Cart{" "}
                      </button>
                    </div>
                  </div>
